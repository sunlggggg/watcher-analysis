package xyz.neolith.watcheranalysis.analysis.eventanalysis;

import org.apache.log4j.Logger;
import xyz.neolith.watcheranalysis.dao.EventDAO;
import xyz.neolith.watcheranalysis.dao.SensitiveportDAO;
...
import static xyz.neolith.watcheranalysis.analysis.eventanalysis.EventType.sameDestAttack;
import static xyz.neolith.watcheranalysis.analysis.eventanalysis.EventType.sameSrcAttack;
import static xyz.neolith.watcheranalysis.analysis.eventanalysis.EventType.sensitivePortAccess;
        ...
        for (int i = 0; i < isFind.length; i++) {
            if (!isFind[i]) {
                ...
                if (i == sameDestAttack.ordinal()) {
                    ...
                    event.setAttackType(sameSrcAttack);
                    events.add(event);
                    ...
                    for (Object s : ports) {
                        if (fwlog.getOriginalDestPort().equals(((Sensitiveport) s).getPort().toString())) {
                            event.setAttackType(sensitivePortAccess);
                            events.add(event);
                            break;
                        }
                    }
